...
- **미팅 정보는 `copy_text`가 아니라 `widget` "이용 안내" 섹션**에 `장소:`/`시간:` Text 노드로 존재(6개 상품 중 5개 일관, 2026-07-07 실측). 어댑터가 `meeting_place`/`meeting_time`로 정규화(F-5). 단 `시간:`은 "예약 확정 후 조율/별도 안내"가 흔해 확정값이 아닐 수 있음.
- **가이드 연락처는 예약 전 응답 계층에 없음**(확정 후 조율). 정규화 필드 없음 — voc-005/025의 긴급 연락 실험은 파트너 입력 스키마 정비가 선행(데이터 측 0단계).
