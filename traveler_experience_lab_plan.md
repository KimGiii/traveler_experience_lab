...
| 성공지표 baseline 부재 | 🔶 준비 완료, 실측 대기 (2026-07-06) | [측정 프로토콜](docs/baseline/protocol.md)·[기록 시트](docs/baseline/baseline_sheet.md) 완비. PE 2~3명 × 과제 2종 실측은 사람 액션 |
...
4. PE 2~3명에게 현재 "문제→시나리오→실험 브리프" 작성 소요시간 **baseline 측정**. → 프로토콜·시트 준비 완료 ([docs/baseline/](docs/baseline/protocol.md)), 실측 대기.
5. 타깃 런타임 **Codex-first + Claude-compatible 듀얼**로 고정 (공용 코어 `core/` + 양쪽 얇은 어댑터).

---

## 9. 성공 지표
- PE의 "문제→이슈/PRD 정리 시간" 50%↓ (**baseline 선측정 전제**).
